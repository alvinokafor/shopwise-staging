...
export default function OrderWithCardConfirmation() {
  ...
  const { mutateAsync, isPending } = useStoreMutation(
    StoreAdapter.verifycheckOutWithCard,
    ""
  );

  const handleVerifyOrder = async function () {
    try {
    ...
    <AppLayout>
      <Box className="flex h-screen flex-col gap-6 items-center justify-center">
        ...
        <Link to={`/orders?tab=ongoing`}>
          <Button disabled={isPending}>Go to Orders</Button>
        </Link>
      </Box>
    </AppLayout>
  );
